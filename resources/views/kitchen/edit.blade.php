...
@section('title', 'Edit Kitchen Record')
...
@section('content')
    <form onsubmit="updateKitchenRecord(event, {{ $record->id }})" class="mx-auto" style="max-width: 560px;">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="order_shop" value="{{ $record->order_shop }}" class="form-control" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Qty</label>
            <input type="number" step="0.01" name="qty" value="{{ $record->qty }}" class="form-control" required>
        </div>
    ...
    <script>
        function updateKitchenRecord(e, id) {
            e.preventDefault();
            $.ajax({
                url: `/kitchen/${id}`,
                method: 'PATCH',
